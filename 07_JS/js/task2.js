...
// Более медленный варинат (на ~60% менее производительный)
// https://jsbench.me/0olbqkbll4/1

/* function findUniq(arr) {
  const preUnic = arr.reduce((total, item) => total + item) - arr[0] * (arr.length - 1);
  return arr.includes(preUnic) ? preUnic : arr[0];
} */
